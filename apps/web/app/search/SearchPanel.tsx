    ...
    >
      <div className="container-standard relative z-10 max-w-4xl text-white">
        <div className="mb-6 text-center md:mb-8">
          <p className="text-primary mb-3 text-xs font-semibold uppercase tracking-[0.2em] md:mb-4 md:text-sm">
            Search
          </p>
          <h1 className="heading-2 mb-3">Find anything on ZephorTech</h1>
          <p className="text-sm text-white/60 md:text-base">
            Services, case studies, blog posts, and more.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 p-5 backdrop-blur md:rounded-2xl md:p-6">
          <div className="relative mb-4">
            ...
            <input
              ...
              className="focus:border-primary focus:ring-primary/40 w-full rounded-xl border border-white/10 bg-transparent py-3 pl-12 pr-28 text-sm text-white transition-all placeholder:text-white/30 focus:outline-none focus:ring-2 md:rounded-2xl md:py-4 md:pr-32 md:text-base"
              value={query}
            ...
            <span className="absolute right-4 top-1/2 flex -translate-y-1/2 items-center gap-1 rounded-lg border border-white/10 bg-white/5 px-2.5 py-1 text-[10px] text-white/60 md:rounded-xl md:px-3 md:text-xs">
              <Command className="h-3 w-3 md:h-3.5 md:w-3.5" />K
            </span>
          </div>
          ...
          <div className="min-h-[100px] md:min-h-[120px]">
            ...
            {status === "idle" && results.length === 0 && query.length >= 2 && (
              <p className="text-sm text-white/60 md:text-base">
                No results found. Try refining your query or use different keywords.
              </p>
            )}
        ...
        {Object.keys(groupedResults).length > 0 && (
          <div className="mt-8 space-y-8 md:mt-10 md:space-y-10">
            {Object.entries(groupedResults).map(([type, items]) => (
              <div key={type}>
                <h2 className="text-primary mb-4 text-xs uppercase tracking-[0.2em] md:text-sm">
                  {type === "service"
                    ? "Services"
                    : type === "case-study"
                      ? "Case Studies"
                ...
                <div className="space-y-3 md:space-y-4">
                  {items.map((result) => (
                    <Link
                      key={result.id}
                      href={result.url}
                      className="hover:border-primary/60 block rounded-xl border border-white/10 bg-white/[0.02] p-4 transition hover:bg-white/[0.04] md:rounded-2xl md:p-5"
                    >
                      <div className="mb-2 flex flex-wrap items-center gap-2 text-xs uppercase tracking-[0.2em] text-white/60 md:gap-3">
                        ...
                        {result.meta && (
                          <span className="text-xs text-white/50 md:text-sm">{result.meta}</span>
                        )}
                      </div>
                      <h3 className="mb-2 text-lg font-semibold text-white md:text-xl">
                        {result.title}
                      </h3>
                      <p className="line-clamp-2 text-sm text-white/60 md:text-base">
                        {result.excerpt}
                      </p>
                    </Link>
                  ))}
